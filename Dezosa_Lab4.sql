...
(select 'order'.*, customer.cus_gender, customer.cud_name from 'order' inner join customer where 'order'.cus_id=customer.cus_id having
...
-- Q7
select product.pro_id,product.pro_name from 'order' inner join supplier_pricing on supplier_pricing.pricing_id='order'.pricing_id inner join
...
(select final.supp_id, supplier.sup_name, final.Average from
(select test2.supp_id, sum(test2.rat_ratstars)/count(test2.rat_ratstars) as Average from 
(select supplier_pricing.supp_id, test.ord_id, test.rat_ratstars from supplier_pricing inner join
(select'order'.pricing_id, rating.rat_ratstars from 'order' inner join rating on rating.'ord_id'='order'.ord_id) as
on test,pricing_id = supplier_pricing.supp_id)
